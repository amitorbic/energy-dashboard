...
export default function AddMetersPage() {
  ...
  function handleSubmit() {
    ...
    localStorage.setItem(
      "pendingRequest",
      JSON.stringify({ action: "add", meters: selectedMeters })
    );
    router.push("/meters/confirm");
  }

  return (
    <Layout title="Add Meters">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Add Meters</h2>
            <p className="text-gray-500 text-sm mt-1">
              Select the meters you want to enroll for service
            </p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => router.push("/meters/add-location")}
              className="flex items-center gap-2 bg-white border border-gray-300 hover:border-blue-400
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            {selected.length > 0 && (
              <div className="px-4 py-3 bg-blue-50 border-t border-blue-100 text-blue-700 text-sm">
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          {selected.length > 0 && (
            ...
            >
              Continue with {selected.length} meter{selected.length > 1 ? "s" : ""}
              <ArrowRight className="h-4 w-4" />
            </button>
          )}
        </div>
