...
**Branch:** master  
**Covers sessions from:** initial architecture planning through Phase 6 enrollment engine refactor (enrollment_masterroll staging table)
...
- ~~`customers`~~ — **never actually existed.** This table name appeared in earlier session notes describing the original single-stage design, but it was never created in any DB (dev or prod). Superseded by `enrollment_masterroll` — see Section 6a below for the current (Option A / two-stage) architecture, which is what's actually running.
...
- Shows every `enrollment_masterroll` row in that batch (see Section 6a for the full staging → activation flow)
- "Mark Active" → `POST /enrollment-engine/activate/{customer_id}`: reads the staged row from `enrollment_masterroll`, INSERTs a new row into `contract_renewal` (status `active`), flips the `enrollment_masterroll` row's status to `active`
- "Mark Cancelled" → sets the `enrollment_masterroll` row's status to `cancelled`
...
### Phase 6a: Two-stage enrollment refactor — `enrollment_masterroll` staging table (Option A)

**Context:** The original Phase 6 design wrote MassRoll enrollment data directly into `contract_renewal` with `status='pending'`, then flipped it to `active` on "Mark Active" (see the now-corrected description above). This meant ERCOT-bound enrollments that were never actually submitted, rejected, or abandoned mid-process still occupied rows in `contract_renewal` — the same table used as the source of truth for active/billable contracts. Option A separates "data staged for ERCOT" from "real, billable contract" into two tables.

**New table — `enrollment_masterroll`:**
Staging table for ERCOT-bound MassRoll enrollments only. A row here means "generated into a MassRoll batch, not yet activated" — it is not a real contract and is never read by billing.

```sql
CREATE TABLE `enrollment_masterroll` (
  ...
  `confirmation_sid` int(11) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `idx_batch_no` (`batch_no`),
  KEY `idx_esi_id` (`esi_id`),
...
**Flow:**
- `POST /enrollment-engine/generate-masterroll` — INSERTs into `enrollment_masterroll` (status `pending`), not `contract_renewal`. Customer-ID daily sequence counts rows from **both** `contract_renewal` and `enrollment_masterroll` (see below) so it can't collide with `create-internal-batch`'s IDs.
- `GET /enrollment-engine/batches/{batch_no}` — reads from `enrollment_masterroll`.
- `POST /enrollment-engine/activate/{customer_id}` — "Mark Active." Reads the staged row from `enrollment_masterroll`, INSERTs a brand-new row into `contract_renewal` (status `active`) with the mapped subset of fields `contract_renewal`'s schema actually supports, then flips the `enrollment_masterroll` row's status to `active`. Guarded: returns 409 if the `enrollment_masterroll` row isn't in `pending`/`submitted` (prevents duplicate `contract_renewal` rows on a retried call, since `contract_renewal` has no unique constraint on `cust_id`). The pre-existing "default" contract auto-creation (0.1300 rate, 30-year end date, `account_type='default'`) is untouched by this refactor.
- `POST /enrollment-engine/cancel/{customer_id}` — sets the `enrollment_masterroll` row's status to `cancelled`. Never touches `contract_renewal`, since a cancelled staged enrollment was never a real contract.
- **`contract_renewal` is now only written on Mark Active** for the MassRoll/ERCOT path — it no longer accumulates pending rows for enrollments that were staged but never activated.

**Internal batches still write directly to `contract_renewal`:**
`POST /enrollment-engine/create-internal-batch` (Renewal / Assignment / B&E) is **unchanged** by this refactor — it still inserts pending rows straight into `contract_renewal` with `account_type='standalone'`, the same way it always has. These contract types don't go through ERCOT/MassRoll, so there's no staging step to add. This means the daily customer-ID sequence has to stay coordinated across three consumers: `enrollment_masterroll` (MassRoll path, pending), `contract_renewal` (internal-batch path, pending), and `contract_renewal` (any path, once active) — `generate-masterroll`'s sequence query sums counts from both tables for exactly this reason.

**Naming inconsistency (intentionally not unified):** `contract_renewal` uses `mtacda_tax_exempt` (no underscore between `mta`/`cda`); `enrollment_masterroll` uses `mta_cda_tax_exempt`. Both names are correct as-is — mapped explicitly at the `activate` boundary, no rename planned.

**Field coverage gap:** `contract_renewal` lacks columns that `enrollment_masterroll` has (`contract_term`, `plan_id2`, `plan_id3`, `priority_code`, `mills`, `lmp`, `tdsp_duns`, `tdsp_name`, `utility_account_number`, `enrol_type`, `confirmation_sid`). These are not lost on activation — the `enrollment_masterroll` row is retained (not deleted) after status flips to `active`, so they remain queryable there.

**Tax Exemptions — no longer admin-only:** The 8 tax exemption fields on the customer profile (`app/pages/customers/[id].tsx`) were previously gated behind admin role in three places (frontend render condition, frontend save-payload whitelist, backend `_ADMIN_ONLY_FIELDS` in `api/routers/contract_renewal.py`). All three gates were removed — any authenticated user can now view and edit tax exemption toggles; changes are still logged to `customer_edit_log`. Other admin-only fields (`energy_rate`, `contract_end_date`, `broker_id`/`broker_name`/`comm_rate`, etc.) are unaffected.

**Cleanup DELETE (Step 7):** After Steps 1–6 were verified end-to-end (TypeScript clean, generate-masterroll/batches/activate/cancel all confirmed against a live test row, tax fields confirmed editable by a non-admin token), a cleanup pass checked `contract_renewal` for orphaned rows from the old single-stage flow (`status='pending' AND batch_no IS NOT NULL AND account_type != 'default'`). Result: zero orphaned rows found — nothing to delete. The dev DB never accumulated stale pending rows under the old flow in a way that needed cleanup.
...
Three schema changes have been applied to the dev DB but not yet to the live Hostinger production DB:
...
- `enrollment_masterroll` table: new staging table for the Phase 6a two-stage enrollment refactor (see Section 2 → Phase 6a above for the full DDL and architecture)

These need to be run manually against `u972964962_orbic` on Hostinger before deploying the corresponding work to production. Repo credentials (`api/.env`) only cover the dev DB (`DB_HOST=localhost`) — there is no committed production connection config, by design.
