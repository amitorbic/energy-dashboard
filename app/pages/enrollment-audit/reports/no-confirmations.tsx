...
export default function NoConfirmations() {
  const [rows, setRows]     = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  const load = () => {
    setLoading(true);
    api.get("/enrollment/reports/no-confirmations")
  ...
  return (
    <EnrollmentLayout title="Enrollment – No Confirmations">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold" style={{ color: "var(--ct-text-primary)" }}>Enrollments with No Confirmations</h2>
        ...
        <div className="overflow-x-auto rounded-[var(--r-md)] border" style={{ borderColor: "var(--ct-border-default)" }}>
          <table className="w-full text-xs">
            <thead className="border-b" style={{ background: "var(--ct-surface-hover)", borderColor: "var(--ct-border-default)" }}>
              <tr>
                {["#","ESID","Company","Broker","Rate","Comm","Term","Start","End","Meter","Status","Comment","Added"].map((h) => (
            ...
            <tbody className="divide-y" style={{ background: "var(--ct-surface)" }}>
              {rows.length === 0 ? (
                <tr><td colSpan={13} className="px-3 py-6 text-center" style={{ color: "var(--ct-text-muted)" }}>No records</td></tr>
              ) : rows.map((r, i) => (
                <tr key={r.esid} className="hover:bg-[var(--ct-surface-hover)]">
                  ...
                  <td className="px-3 py-2 max-w-[150px] truncate" style={{ color: "var(--ct-text-muted)" }}
                    dangerouslySetInnerHTML={{ __html: (r.enrollment_status_comment || "").replace(/<br\s*\/?>/gi, " ") }} />
